...
class InstanceChecker:
    @classmethod
    def is_instance(cls, value: Any, type_: Type) -> bool:
        try:
        ...
        if type_ == Any:
            ...
            return any(cls.is_instance(value, t) for t in extract_generic(type_))
            ...
            if isinstance(value, tuple) and is_tuple(type_):
                ...
                if len(tuple_types) == 1 and tuple_types[0] == ():
                    ...
                    return all(cls.is_instance(item, tuple_types[0]) for item in value)
                else:
                    if len(tuple_types) != len(value):
                        return False
                    return all(cls.is_instance(item, item_type) for item, item_type in zip(value, tuple_types))
            if isinstance(value, Mapping):
                key_type, val_type = extract_generic(type_, defaults=(Any, Any))
                for key, val in value.items():
                    if not cls.is_instance(key, key_type) or not cls.is_instance(val, val_type):
                        return False
                return True
            return all(cls.is_instance(item, extract_generic(type_, defaults=(Any,))[0]) for item in value)
        elif is_new_type(type_):
            return cls.is_instance(value, extract_new_type(type_))
        elif is_literal(type_):
            return value in extract_generic(type_)
        elif is_init_var(type_):
            return cls.is_instance(value, extract_init_var(type_))
...
is_instance = InstanceChecker.is_instance
